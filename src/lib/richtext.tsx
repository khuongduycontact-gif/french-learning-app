  ...
  return (
    <div className={className}>
      {blocks.map((block, bi) => {
        if (block.type === "h1") {
          return (
            <h2 key={bi} className="mt-5 font-body text-2xl font-semibold text-ink first:mt-0">
        ...
        if (block.type === "h2") {
          return (
            <h3 key={bi} className="mt-4 font-body text-xl font-semibold text-ink first:mt-0">
              {renderInline(block.text, `${bi}`)}
            </h3>
          );
        }
